...
1. Escribe en texto plano, conversacional y amigable. Usa puntos y comas para separar tus ideas y generar pausas naturales al hablar.
2. Al inicio de la conversacion, el sistema ya le pregunto al usuario su nombre. Cuando el usuario te responda diciendo como se llama, saludalo por su nombre y se amable. A partir de ese momento, recuerda su nombre y usalo cada vez que le hables. Si el usuario no te ha dicho su nombre, no lo llames por ningun nombre ni le digas que no sabes su nombre, simplemente habla normalmente."
3. NO le pidas el horario hasta que te haya dicho su nombre (la primera vez que lo haga).
"""

# hacer un print de siguiente cuando el usuario no tenga calendario y/o horario: "Para dar inicio al proceso. ¿Me podrias proporcionar tu horario?. Puedes usar el boton Subir Horario de abajo."

sesion_chat = cliente_ai.chats.create(
    model="gemini-2.5-flash",
    config=types.GenerateContentConfig(
        system_instruction=instrucciones_sistema, temperature=0.6
    ),
)
...
def procesar_mensaje_texto(texto, ui_callback):
    ...
    try:
        ...
        threading.Thread(
            target=reproducir_audio, args=(texto_limpio,), daemon=True
        ).start()
    except Exception as e:
...
def procesar_audio_microfono(ui_callback, ui_status_callback):
    ...
    try:
        ...
        respuesta = sesion_chat.send_message(
            [types.Part.from_bytes(data=audio_bytes, mime_type="audio/wav"), prompt_voz]
        )
...
def procesar_imagen_horario(ruta_imagen, ui_callback):
    ...
    try:
        ...
        if match_json:
            ...
            mensaje_conversacional_sucio = re.sub(
                r"```json.*?```", "", texto_ia, flags=re.DOTALL
            ).strip()
            mensaje_conversacional = limpiar_texto_markdown(
                mensaje_conversacional_sucio
            )
                ...
                with SesionBD() as sesion:
                    for c in datos.get("clases", []):
                        nueva_clase = ClaseHorario(
                            ...
                            dia=c.get("dia", ""),
                            maestro=c.get("maestro", "Sin maestro"),
                            contacto_maestro=c.get("contacto", "Sin contacto"),
                            salon=c.get("salon", "Por definir"),
                            hora_inicio=c.get("hora_inicio", "00:00"),
        ...
        threading.Thread(
            target=reproducir_audio, args=(mensaje_conversacional,), daemon=True
        ).start()
...
class HermesGUI:
    def __init__(self, root):
        ...
        ttk.Button(frame_botones, text="Enviar", command=self.enviar_texto).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(frame_botones, text="Voz", command=self.enviar_voz).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(
            frame_botones, text="Subir Horario", command=self.abrir_explorador
        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            frame_botones,
            text="Mi Horario Detallado",
            command=self.abrir_visor_horario_ampliado,
        ...
        self.entrada_texto.pack(
            side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 10), ipady=8
        )
        ...
        threading.Thread(
            target=reproducir_audio, args=(msg_bienvenida,), daemon=True
        ).start()
    ...
    def enviar_texto(self):
        ...
        threading.Thread(
            target=procesar_mensaje_texto,
            args=(texto, self.actualizar_ui_desde_hilo),
            daemon=True,
        ).start()

    def enviar_voz(self):
        threading.Thread(
            target=procesar_audio_microfono,
            args=(self.actualizar_ui_desde_hilo, self.actualizar_estado_desde_hilo),
            daemon=True,
        ).start()

    def abrir_explorador(self):
        ruta = filedialog.askopenfilename(
            title="Selecciona la imagen de tu horario",
            filetypes=[("Imagenes", "*.png *.jpg *.jpeg")],
        )
        if ruta:
            threading.Thread(
                target=procesar_imagen_horario,
                args=(ruta, self.actualizar_ui_desde_hilo),
                daemon=True,
            ).start()
    ...
    def abrir_visor_horario_ampliado(self):
        ...
        tabla.column("Materia", width=250)
        tabla.column("Dia", width=100, anchor="center")
        tabla.column("Maestro", width=150)
        tabla.column("Salon", width=80, anchor="center")
        tabla.column("Hora Inicio", width=80, anchor="center")
        tabla.column("Hora Fin", width=80, anchor="center")

        scrollbar_y = ttk.Scrollbar(
            frame_tabla, orient=tk.VERTICAL, command=tabla.yview
        )
        tabla.configure(yscrollcommand=scrollbar_y.set)
        scrollbar_y.pack(side=tk.RIGHT, fill=tk.Y)
        tabla.pack(expand=True, fill=tk.BOTH, side=tk.LEFT)

        frame_contacto = tk.Frame(
            ventana_horario, bg="#CCCCCC", relief="solid", borderwidth=1
        )
        frame_contacto.pack(fill=tk.X, padx=15, pady=(0, 15))

        tk.Label(
            frame_contacto,
            text="Informacion de Contacto del Maestro",
            bg="#CCCCCC",
            fg="black",
        ...
        def recargar_tabla():
            ...
            with SesionBD() as sesion:
                clases = (
                    sesion.query(ClaseHorario)
                    .filter_by(usuario_id=usuario_actual_id)
                    .all()
                )
                for c in clases:
                    tabla.insert(
        ...
        def abrir_edicion_completa():
            ...
            tk.Label(
                edit_win,
                text="Modifica los datos que necesites:",
                bg="#CCCCCC",
                fg="black",
                font=("Segoe UI", 12, "bold"),
            ).pack(pady=20)
            frame_form = tk.Frame(edit_win, bg="#CCCCCC")
            ...
            def create_entry(lbl_text, row, val):
                ...
                ent = tk.Entry(frame_form, font=("Segoe UI", 10), width=35)
                ent.insert(0, val)
            ...
            listbox_contactos = tk.Listbox(
                frame_contactos, height=4, width=35, font=("Segoe UI", 9)
            )
            ...
            ent_nuevo_contacto = tk.Entry(
                frame_add_contacto, font=("Segoe UI", 9), width=15
            )
            ...
            def guardar_cambios_completo():
                ...
                n_con = (
                    " | ".join(contactos_finales)
                    if contactos_finales
                    else "Sin contacto"
                )
                ...
                with SesionBD() as s:
                    ...
                    if c_db:
                        if c_db.materia != n_mat:
                            cambios.append(f"Materia a '{n_mat}'")
                        if c_db.dia != n_dia:
                            cambios.append(f"Dia a '{n_dia}'")
                        if c_db.salon != n_sal:
                            cambios.append(f"Salon a '{n_sal}'")
                        if c_db.maestro != n_mae:
                            cambios.append(f"Maestro a '{n_mae}'")
                        if c_db.contacto_maestro != n_con:
                            cambios.append(f"Contactos actualizados")
                        if c_db.hora_inicio != n_hi or c_db.hora_fin != n_hf:
                            cambios.append(f"Horario a '{n_hi}-{n_hf}'")

                        c_db.materia = n_mat
                        c_db.dia = n_dia
                        c_db.maestro = n_mae
                        c_db.contacto_maestro = n_con
                        c_db.salon = n_sal
                        c_db.hora_inicio = n_hi
                        c_db.hora_fin = n_hf
                        s.commit()
                ...
                if cambios:
                    ...
                    threading.Thread(
                        target=reproducir_audio, args=(msg,), daemon=True
                    ).start()

            ttk.Button(
                edit_win, text="Aceptar Cambios", command=guardar_cambios_completo
            ).pack(pady=20)

        frame_acciones = tk.Frame(ventana_horario, bg="#CCCCCC")
        frame_acciones.pack(fill=tk.X, ipady=10)
        frame_btn_cont = tk.Frame(frame_acciones, bg="#CCCCCC")
        frame_btn_cont.pack()

        tk.Button(
            frame_btn_cont,
            text="Editar Seleccion",
            command=abrir_edicion_completa,
            font=("Segoe UI", 10),
            bg="white",
            fg="black",
            padx=10,
        ...
        tk.Button(
            frame_btn_cont,
            text="Cerrar Panel",
            command=ventana_horario.destroy,
            font=("Segoe UI", 10),
            bg="white",
            fg="black",
            padx=10,
    ...
    def actualizar_ui_desde_hilo(self, texto, emisor):
        self.root.after(0, self.agregar_mensaje, texto, emisor)

    def actualizar_estado_desde_hilo(self, texto):
        self.root.after(0, self.cambiar_estado, texto)
